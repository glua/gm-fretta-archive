...
function ENT:AddScore( ent )
	...
	if table.HasValue( GAMEMODE.PropModels, model ) then
		team.AddScore( TEAM_YELLOW, 1 )
		
		if ( team.GetScore( TEAM_YELLOW ) - 1 <= team.GetScore( TEAM_BLUE ) ) &&( team.GetScore( TEAM_YELLOW ) > team.GetScore( TEAM_BLUE ) ) then
			GAMEMODE:YellowTakesLead()
		end
		
		if ( ent:GetNWEntity( "L_Affector" ) and ent:GetNWEntity( "L_Affector" ):IsPlayer() ) then
			if ent:GetNWEntity( "L_Affector" ):Team() == TEAM_YELLOW then
				ent:GetNWEntity( "L_Affector" ):AddDeliveries( 1 )
			elseif ent:GetNWEntity( "L_Affector" ):Team() == TEAM_BLUE then
				ent:GetNWEntity( "L_Affector" ):AddSteals( -1 )
			end
		end
	elseif table.HasValue( GAMEMODE.LargeModels, model ) then
		team.AddScore( TEAM_YELLOW, 2 )
		
		if ( team.GetScore( TEAM_YELLOW ) - 2 <= team.GetScore( TEAM_BLUE ) ) &&( team.GetScore( TEAM_YELLOW ) > team.GetScore( TEAM_BLUE ) ) then
			GAMEMODE:YellowTakesLead()
		end
	...
	boosh:SetScale( 1 )
	util.Effect( "yellow_point", boosh )
