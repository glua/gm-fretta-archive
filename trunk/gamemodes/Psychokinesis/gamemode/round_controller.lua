...
function GM:RoundTimerEnd()
	...
	if ( !GAMEMODE.TeamBased ) then
		...
		if ply then
			GAMEMODE:RoundEndWithResult( ply, ply:GetName() .. " wins!" )
			UTIL_FreezeAllPlayers()
		else
...
function GM:SelectCurrentlyWinningPlayer()
	...
	for k,v in pairs( player.GetAll() ) do
		if v:Frags() >= topscore and v:Team() != TEAM_CONNECTING and v:Team() != TEAM_SPECTATOR then
			
			if((v:Frags() == topscore) and !(winner == nil))then
				if(v:Deaths() < winner:Deaths())then
					winner = v
					topscore = v:Frags()
				end
			else
