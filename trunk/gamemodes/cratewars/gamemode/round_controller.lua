...
function GM:OnRoundStart( num )
	...
	for _,ply in pairs(player.GetAll()) do
		ply:SendLua('PlaySound("'..SOUND_FIGHT..'")')
	end
		...
		if(ply:Team() != TEAM_SPECTATOR) and (ply:Team() != TEAM_UNASSIGNED)then
			if(result != -1)then
				if(ply:Team() == result)then
					ply:SendLua('PlaySound("'..SOUND_WIN..'")')
				else
					ply:SendLua('PlaySound("'..SOUND_LOSE..'")')
				end
			else
				ply:SendLua('PlaySound("'..SOUND_STALE..'")')
			end
		end
	end
...
function GM:OnPreRoundStart( num )
	...
	UTIL_UnFreezeAllPlayers()
	GAMEMODE:CheckForNeutrals()
	GAMEMODE:CheckForBuildings()
	GAMEMODE:ResetAllPlayersPoints()
	team.SetScore(TEAM_RED,0)
	team.SetScore(TEAM_BLUE,0)
	for _,ply in pairs(player.GetAll()) do
		ply:SendLua('PlaySound("'..SOUND_BUILD..'")')
		ply:SetFrags(0)
		ply:SetDeaths(0)
		GAMEMODE:resetStats(ply)
	end
end
...
function GM:RoundStart()
	...
	SetGlobalBool( "InRealRound", true )
end
...
function GM:RoundEnd()
	...
	SetGlobalBool( "InRealRound", false )
