...
surface.CreateFont( "Tahoma", 22, 500, true, false, "ScoreFont" )
...
function GM:PaintCash()
	...
	local CashIndicator = vgui.Create( "DHudUpdater" )
		...
		CashIndicator:SetValueFunction( function()
											Cash = math.Approach( ( Cash or 0 ), LocalPlayer():GetNWInt( "Cash", 0 ), 5 )
											return "$"..Cash 
										end )
	...
	local RedScore = vgui.Create( "DHudUpdater" )
		...
		RedScore:SetValueFunction( function() 
										RedCash = math.Approach( ( RedCash or 0 ), GetGlobalInt( "TeamCash"..TEAM_RED ), 10 )
										return "$"..RedCash
									end )
	...
	local BlueScore = vgui.Create( "DHudUpdater" )
		...
		BlueScore:SetValueFunction( function() 
										BlueCash = math.Approach( ( BlueCash or 0 ), GetGlobalInt( "TeamCash"..TEAM_BLUE ), 10 )
										return "$"..BlueCash
									end )
