...
SWEP.ScopedWeapon				= false;
SWEP.IronSightsPos  			= Vector( 0, 0, 0 );
SWEP.IronSightAng				= Vector( 0, 0, 0 );
SWEP.IronsightAccuracy			= 2.5;
SWEP.IronsightTime				= 0.35;

SWEP.ShellType					= SHELL_762NATO;
...
function SWEP:Deploy()
	...
	self:SendWeaponAnim( self:GetDrawActivity() )
...
function SWEP:CanShootWeapon()
	return ( ( CurTime() >= self.NextPrimaryAttack || self.BurstQueue > 0 ) && self.Weapon:GetNetworkedBool( "RunningAnims", false ) == false && self.Weapon:GetNetworkedBool( "reload", false ) == false );
end
...
function SWEP:PrimaryAttack()
	...
	if( ( ( CLIENT && IsFirstTimePredicted() ) || SERVER ) and ValidEntity( self.Owner ) and self:CanShootWeapon() ) then
...
function SWEP:Reload()
	...
	if( self.NextPrimaryAttack >= CurTime() || self.Weapon:GetNetworkedBool( "reload", false ) == true ) then return end
	...
	self.Weapon:CustomReload( self:GetReloadActivity() );
...
function SWEP:Think()
	...
	elseif( self.AutoReload && self.Weapon:Clip1() <= 0 ) then
		self:Reload()
	end
		
	if( SERVER || CLIENT && self:GetOwner() == LocalPlayer() ) then // only predict localplayer, prevents spectator bugs
...
function SWEP:IdleThink()
	...
	self.Weapon:SendWeaponAnim( self:GetIdleActivity() )
...
function SWEP:SetIronsights( b )
	...
	if( self.Weapon:GetNetworkedBool( "reload", false ) == true && self.ScopedWeapon && b == true) then
		return;
	end
	...
	if( SERVER ) then
		if( self.HasIronsights == true and self.Owner ) then
			if( b == true ) then
				if( self.IronsightDelayFOV or self.ScopedWeapon ) then
