...
function SWEP:CustomAmmoDisplay()
	...
	if( self.Primary.ClipSize < 1 && self.Primary.Ammo == "none" && self.CustomSecondaryAmmo ) then
		self.AmmoDisplay.PrimaryClip = self.Weapon:CustomAmmoCount();
		self.AmmoDisplay.PrimaryAmmo = -1;
	else
		if( self.Primary.ClipSize > 0 ) then
			self.AmmoDisplay.PrimaryClip = self.Weapon:Clip1()
			self.AmmoDisplay.PrimaryAmmo = self.Weapon:Ammo1()
		end
		
		if( self.CustomSecondaryAmmo ) then
			self.AmmoDisplay.SecondaryAmmo = self.Weapon:CustomAmmoCount();
		elseif( self.Secondary.ClipSize > 0 ) then
			self.AmmoDisplay.SecondaryAmmo = self.Weapon:Ammo2()
		end
	end
